...
// Copied and adapted search logic from src/lib/product-search.ts to pure JS

const STOP_WORDS = new Set([
  'a', 'as', 'c', 'com', 'da', 'das', 'de', 'diant', 'do', 'dos',
  'e', 'em', 'o', 'os', 'p', 'para', 's', 'sem', 'todos', 'tras', 'traseiro',
]);
...
function indexProduct(produto) {
  ...
  const searchable = normalizeSearchText([
    name, brand, category, reference, code, ncm, produto.unidade, aliasText
  ].join(' '));
...
function scoreTermVariant(item, term, loose) {
...
function scoreProduct(item, terms, normalizedQuery, compactQuery, loose = false) {
  let score = 0;
  let matchedTerms = 0;
...
function sortProducts(produtos, order) {
  const sorted = [...produtos];
  sorted.sort((left, right) => {
    if (order === 'menor-preco') {
      const lp = left.preco === null || left.preco === undefined ? Infinity : left.preco;
      const rp = right.preco === null || right.preco === undefined ? Infinity : right.preco;
      return lp - rp || byName(left, right);
    }
    if (order === 'maior-preco') {
      const lp = left.preco === null || left.preco === undefined ? -Infinity : left.preco;
      const rp = right.preco === null || right.preco === undefined ? -Infinity : right.preco;
      return rp - lp || byName(left, right);
    }
...
function searchProducts(produtos, query) {
...
// Main Test Logic
function runTest() {
  const inventarioPath = path.join(__dirname, 'src', 'data', 'produtos-inventario.json');
  console.log('Verificando se o arquivo do inventário existe...');
  if (!fs.existsSync(inventarioPath)) {
    console.error('Arquivo de inventário não encontrado!');
    process.exit(1);
  }

  console.log('Lendo inventário...');
  ...
  const testQueries = [
    ...
    'cremalheira',
    'volvo fh',
    '9796977098', // exact reference/code check
    'sensor abs',
    'mbb'
  ];
  ...
  testQueries.forEach((query) => {
    ...
    const results = searchProducts(produtos, query);
    console.log(`Encontrados: ${results.length} resultados.`);
    if (results.length > 0) {
      console.log('Top 3 resultados:');
      results.slice(0, 3).forEach((p, idx) => {
        console.log(`  ${idx+1}. [${p.id}] ${p.nome} | Cod: ${p.codigoInterno} | Ref: ${p.referencia} | Marca: ${p.marca || 'N/A'}`);
      });
    }
  });
}
